...
export default function FriendsPage() {
  ...
  const handleAcceptRequest = async (requestId) => {
    try {
      ...
      toast.success(language === 'hi' ? 'अनुरोध स्वीकार किया' : 'Request accepted');
      fetchFriends();
    } catch (error) {
  ...
  const handleRemoveFriend = async (friendId) => {
    try {
      ...
      toast.success(language === 'hi' ? 'दोस्त हटाया गया' : 'Friend removed');
              ...
              <div className="space-y-4">
                <div className="flex gap-2">
                  ...
                  <Button 
                    ...
                    disabled={searching}
                    className="h-12 px-4 bg-indigo-600 hover:bg-indigo-700 rounded-xl"
                ...
                <div className="space-y-2 max-h-64 overflow-y-auto">
                  ...
                  {searchResults.length === 0 && searchQuery && !searching && (
                    <p className="text-center text-slate-500 py-4">{t('noResults')}</p>
                  )}
                </div>
              </div>
            </DialogContent>
          ...
          <div className="mb-6">
            ...
            <div className="space-y-3">
              {pendingRequests.map((request) => (
                ...
                >
                  <Card className="border-0 shadow-sm rounded-2xl bg-amber-50" data-testid={`pending-request-${request.request_id}`}>
                    <CardContent className="p-4 flex items-center gap-4">
                      <Avatar className="w-12 h-12">
                        <AvatarImage src={request.avatar_url || `https://api.dicebear.com/7.x/avataaars/svg?seed=${request.name}`} />
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-slate-500">
                          {language === 'hi' ? 'दोस्ती का अनुरोध' : 'Friend request'}
                        </p>
                      </div>
                      <div className="flex gap-2">
                        <Button
                          ...
                          className="w-10 h-10 bg-green-600 hover:bg-green-700 rounded-full"
        ...
        {loading ? (
            ...
            {friends.map((friend, index) => (
              ...
              >
                <Card className="border-0 shadow-sm rounded-2xl" data-testid={`friend-${friend.id}`}>
                  <CardContent className="p-4 flex items-center gap-4">
                    <Avatar className="w-12 h-12">
                      <AvatarImage src={friend.avatar_url || `https://api.dicebear.com/7.x/avataaars/svg?seed=${friend.name}`} />
                      <AvatarFallback className="bg-indigo-100 text-indigo-600">
                    ...
                    <Button
                      ...
                      className="w-10 h-10 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-full"
          ...
          <Card className="border-0 shadow-sm rounded-2xl">
            <CardContent className="p-8 text-center">
              ...
              <p className="text-slate-500">{t('noFriends')}</p>
            </CardContent>
          </Card>
        )}
      </main>
